                    ...
                    <div class="ibox">
                        ...
                        <div class="ibox-content" style="min-height:70vh;">
							...
							<div class="col-md-12">	<hr/></div>
								<div class="claerfix"></div>
								<div class="col-md-12">
									<form action="{{action('admin\ManageSpecialOrderController@adminUpdateSpecialOrder')}}" method="POST" enctype="multipart/form-data" class="productss" id="order_edit">
										<hr/>
											...
											<div class="form-group">
												...
												<select data-placeholder="Choose a Product..." class="select2_demo_3 form-control" style="width: 100%"  tabindex="4" name="product_name"  required id="product_name">
													...
													<?php 
														...
														?>
															<option value="<?php echo $getProduct->product_id; ?>" <?php if($getProduct->product_id==$orderData->product_id){echo 'selected';} ?>>{{$getProduct->productName}}</option>
													<?php
														}
													?>
												</select>
										...
										<div class="col-sm-4">
											...
											<div class="form-group">
												<label class="control-label">Product Side Panel Color</label>
												<select class="selectpicker form-control"  name="product_side_color" id="product_color_filter" >
													<option value="">Select Side Panel Color</option>
													<?php
													$selected='';
														foreach($coloesidejson as $k=>$v){
															if($k == $orderData->product_side_color){
										...
										<div class="col-sm-12">
											<div class="col-sm-2"> 
												<label class="control-label">Attributes :</label><br/>
											</div><div class="clearfix"></div>
										 
												<?php
													 $getProdAttrs=DB::table('product_attribute')->where('product_id','=',$orderData->product_id)->get();
													 
													  $checked='';
													  foreach($getProdAttrs as $getProdAttr){
														 $attrname=DB::table('attribute')->where('attributeID','=',$getProdAttr->attributeID)->first();
														 
														 if(!empty($orderData->attribute)){
															$attrid=explode(',',$orderData->attribute);
															/* for($i=0;$i<count($attrid);$i++){
																
																if(isset($attrid[$i]) && $attrname->attributeID == $attrid[$i] ){
																	$checked='checked';
															...
															if(in_array($attrname->attributeID, $attrid) ){
																	$checked='checked';
											...
											<div class="form-group" id="attributes"><div class="i-checks"><label> <input type="checkbox" name="attributeid[]" '.$checked.' value="'.$attrname->attributeID.'" class="form-control" > <i></i>'.$attrname->attributeName.'</label></div></div>
										</div>';
										...
										<div class="col-sm-4">
											<div class="form-group">
												...
												<input type="submit" class="btn btn-w-m btn-primary" id="stockupdate" value="Update" />
												 <a href="{{URL::to('/admin/specialorderslist')}}" class="btn btn-default pull-right"><i class="fa fa-arrow-left"></i> Back </a>
											</div>
										</div>
									</form>
								</div>
		...
		<script>
			$(function(){
					...
					function getProductColor(){
						...
						({
							type: "POST",
							url: "{{URL::to('admin/ajax/log/getproductscoloreditspeacilorder')}}",
					...
					function getattribute(productID){
						...
						if(productID !=''){
							...
							({
								...
								data: {data_productID: productID, _token: _token},
								cache: false,
