...
class WarrantyCustomerController extends Controller
{
    ...
    public function index()
    {
        ...
        if(isset($id) && ($id != 0))
        {
            $data['total_pending_warranty'] = WarrantyProduct::select('id')->where('user_id','=',$id)->where('role','=','customer')->where('warranty_status','!=','complete')->orderBy('id','DESC')->count();
            $data['total_complete_warranty'] = WarrantyProduct::select('id')->where('user_id','=',$id)->where('role','=','customer')->where('warranty_status','=','complete')->orderBy('id','DESC')->count();

            $result_warrantyList = WarrantyProduct::select('*')->where('user_id','=',$id)->where('role','=','customer')->where('warranty_status','!=','complete')->orderBy('id','DESC')->get();
    ...
    public function warrantyAdd()
    {
        ...
        if(isset($id) && ($id != 0))
        {
            ...
            $result_customer = Customer::select('*')->where('id','=',$id)->orderBy('id','ASC')->first();
            $data['customer_data'] = $result_customer;

            return View::make('warranty/customer/customerWarrantyAdd',$data);
        }else{
        ...
        if(isset($id) && ($id != 0))
        {
                ...
                && (($data_post['method_process'] == 'add') || ($data_post['method_process'] == 'edit')) )
            {
                ...
                if ($validator->fails())
                {
                    ...
                    return Redirect::to('/customer/warrantyadd');
                }else{
                    $update_post = array();
                    ...
                    if(!empty($result_warranty_product))
                    {
                        ...
                        if(!empty($staff_emailID_result))
                        {
                            ...
                            $warranty_info = array(
                                'title_message' => 'Customer Change Warranty Product Details',
                                'to_name' => $assign_name,
                    ...
                    return Redirect::to('/customer/warranty');
                }
            }else{
                Session::flash('operationFaild','Some thing went wrong!');
                if($data_post['method_process'] == 'add')
                ...
                {
                    return Redirect::to('/customer/warrantyedit/'.$data_post['warranty_uniqueID']);
                }else{
                    return Redirect::to('/customer/warranty');
                }
            }
    ...
    public function warrantyEdit($editid)
    {
        ...
        if(isset($id) && ($id != 0))
        {
            $result = DB::table('warrantyproduct')->where('warranty_uniqueID','=',$editid)->where('user_id','=',$id)->where('role','=','customer')->first();
            if(!empty($result))
            {
                ...
                return View::make('warranty/customer/customerWarrantyEdit',$data);
            }else{
                return Redirect::to('customer/warranty');
            }
        }else{
    ...
    public function getFileWarranty()
    {
        ...
        if(isset($id) && ($id != 0))
        {
            $result = DB::table('warrantyproduct')->where('warranty_uniqueID','=',$data_post['data_warranty_uniqueID'])
                ->where('user_id','=',$id)->where('role','=','customer')->first();
    ...
    public function noteDataForm()
    {
        ...
        if(isset($id) && ($id != 0))
        {
            ...
            if(!empty($result))
            {
                ...
                return View::make('warranty/customer/noteWarranty',$data);
            }else{
                echo '444';
                exit;
            }
