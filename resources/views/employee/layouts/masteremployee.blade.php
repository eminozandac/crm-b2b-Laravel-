...
<body>
    <div id="wrapper">
		<nav class="navbar-default navbar-static-side" role="navigation">
			@include('employee.includes.sidebarmenu')
	    </nav>
			...
			<div class="row border-bottom">
				<nav class="navbar navbar-static-top" role="navigation" style="margin-bottom: 0">
					...
					<ul class="nav navbar-top-links navbar-right">
                                    ...
                                    @foreach($today_task as $key_today_task => $value_today_task)
                                        <li>
                                            <a href="javascript:void(0);" onclick="displayAssignTask('{{ 'notificationlist_'.$no }}')" id="notificationlist_{{ $no }}" data-title="{{ $today_task_list_data[$no]['title'] }}"  data-assigndate="{{ $today_task_list_data[$no]['assign_date'] }}"
                                               data-completiondate="{{ $today_task_list_data[$no]['completion_date'] }}" data-taskassign="{{ $today_task_list_data[$no]['task_employee'] }}"
                        ...
                        <li>
							<a class="count-info" title="User Profile" href="{{action('employee\EmployeeController@profile')}}">
								<i class="fa fa-user" aria-hidden="true"></i>
							</a>
						</li>
						<li>
							<a title="Logout" href="{{action('employee\EmployeeController@logout')}}">
