...
@section('contentPages')
			...
			<!--------------------------------Pending Orders--------------------------------------->
										...
										<div id="tab-1" class="tab-pane <?php  if(!Session::get('opentab')){ echo'active';} if(Session::get('opentab')=='pending'){ echo'active';}?>"> 
											...
											<div class="col-md-2">
												<form class="m-t form-horizontal"  role="form" method="post" action="{{action('admin\OrderController@swapOrder')}}" id="form_customer_profile" >
											...
											<div class="col-md-10">
												<form class="m-t form-horizontal"  role="form" method="post" id="bookedform" action="{{action('admin\OrderController@bookedForDeliveryOrder')}}" id="form_customer_profile" >
													<input type="hidden" name="_token"  value="<?php echo csrf_token(); ?>"/>
													<div class="col-md-4">
...
@section('pagescript')
	...
	$(function() 
	{
		...
		var completeordersingle_table = $('#completeordersingle_table').DataTable({
			...
			"serverSide": true,
			"ajax": {
				url: "{{ URL::to('admin/adminorderlistcompleteordersingle') }}",
				data: function (d) {
				}
			},
			"columns": [
