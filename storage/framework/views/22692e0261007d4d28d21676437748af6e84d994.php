...
<?php $sessionData=Session::get('dealerLog');$id = $sessionData['dealerID']; ?>
							...
							<div class="ibox">
								...
								<input type="hidden" name="_token"  id="token" value="<?php echo csrf_token(); ?>"/>
									<table class="table table-striped table-bordered table-hover dataTables-example" id="order_table" style="width:100%;" >
										...
										<tbody>
											<?php
												$getOrders=DB::table('accessory_order')->where('dealerID','=',$id)->where('deleted_at','=',NULL)->get();
												foreach($getOrders as $order){
													
													$dealername=DB::table('dealer')->where('id','=',$order->dealerID)->first();
													
													$detailURL=URL::to('dealer/accessoryorderdetail', base64_encode($order->accessory_order_ID));
											...
											<tr>
												...
												<td>
													<a href="<?php echo $detailURL; ?>" data-toggle="tooltip" title="Edit order" class="btn btn-xs btn-default"><i class="fa fa-eye"></i></a>
													<?php 
