                    ...
                    <div class="ibox">
                        ...
                        <div class="ibox-content" >
                            <div class="table-responsive">
								...
								<div class="col-md-12">
								<?php 
									 if($getOrderTran->orderStatus=='pending'|| $getOrderTran->orderStatus=='booked in for delivery' || $getOrderTran->orderStatus=='collection'){
								?>
									<form action="{{action('admin\OrderController@adminupdateorder')}}" method="POST" enctype="multipart/form-data" class="productss" id="order_edit">
										<hr/>
										<div class="col-sm-12"><h2>Order Edit</h2><br/></div>
										<input type="hidden" name="_token"  id="token" value="<?php echo csrf_token(); ?>"/>
										<input type="hidden" name="pagedata"  id="pagedata" value="<?php echo $pagedata; ?>" />
										...
										<input type="hidden" name="opentab"  id="opentab" value="<?php if(isset($orderID[2]) && !empty(isset($orderID[2]))){
											
											echo $orderID[2];} ?>"/>
									<?php 
									if($getOrderTran->specialOrderID == 0){
									?>
											...
											<div class="form-group">
												...
												<select data-placeholder="Choose a Product..." class="select2_demo_3 form-control" style="width: 100%"  tabindex="4" name="product_name"  required id="product_name">
													...
													<?php 
														...
														?>
															<option value="<?php echo base64_encode($getProduct->product_id); ?>" <?php if($getProduct->product_id==$getOrderTran->product_id){echo 'selected';} ?>>{{$getProduct->productName}}</option>
													<?php
														}
													?>
												</select>
									...
									<?php 
											...
											?>
											<input type="hidden" name="product_name"  value="<?php echo base64_encode($getOrderTran->product_id); ?>"/>
											<input type="hidden" name="product_color"  value="<?php echo $getOrderTran->product_color; ?>"/>
											<input type="hidden" name="productbatch"  value="<?php echo $getOrderTran->batch; ?>"/>
											<input type="hidden" name="productStatus"  value="<?php echo $getOrderTran->qtystatus; ?>"/>
									<?php
										}
									?>
										...
										<div class="col-sm-4">
											<div class="form-group">
												...
												<select class="form-control" name="orderStatus" required id="orderStatus" >
													<option value="">Select Status</option>
													<option <?php if($getOrderTran->orderStatus=='pending'){echo 'selected=selected';}?> value="pending">Pending</option>
													<option <?php if($getOrderTran->orderStatus=='booked in for delivery'){echo 'selected=selected';}?> value="booked in for delivery">Booked in for delivery</option>
													<option <?php if($getOrderTran->orderStatus=='collection'){echo 'selected=selected';}?> value="collection">Collection</option>
												...
												<select data-placeholder="Select Accessories..." class="chosen-select" style="width: 100%" tabindex="4" name="accessory_name" id="accessory_name">
												 <option value="" selected>Select Accessory</option>
													<?php 
														...
														?>
															<option value="<?php echo base64_encode($productAcc->accessoryID); ?>" >{{$productAcc->accessory_name}}</option>
													<?php
														}
													?>
												</select>
										...
										<div class="col-sm-4">
											<div class="form-group">
												...
												<a href="{{URL::to('/admin/orderList')}}" class="btn btn-default pull-right"><i class="fa fa-arrow-left"></i> Back </a>
											</div>
										</div>
			...
			<script>
				$(function(){
					...
					function getProductBatch(nobatch){
						...
						({
							type: "POST",
							url: "{{URL::to('admin/ajax/log/getproductsbatcheditorder')}}",
					...
					function getProductStatus(nostatus){
						...
						({
							type: "POST",
							url: "{{URL::to('admin/ajax/log/getproductsstatuseditorder')}}",
					...
					function getProductColor(){
						...
						({
							type: "POST",
							url: "{{URL::to('admin/ajax/log/getproductscoloreditorder')}}",
					...
					$('#orderStatus').change(function(){
						...
						if(orderStatus == 'booked in for delivery' ||  orderStatus=='invoiced' ){
